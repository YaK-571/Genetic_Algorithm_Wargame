...
void UGame_Instance_Wargame::f_start ()
{
	...
	//если бой первый, или состав армии помен€лс€, то...
	if ((number_battle == 1)||(smena_stroja==1))
	{
		...
		*/
		...
		//применить построение дл€ всех отделений
	...
	else
	{
		...
		//загрузить наиболее эффективные построени€ из предыдущего бо€
...
{
	//первые 2 построени€ остаютс€ такими же, как и 2 наиболее эффективных построений в бою
	// остальные - получаютс€ с помощью мутаций
	for (int number = 2; number <= 4; number++)
	{
		//повторить дл€ n-го числа юнитов в отделении
		for (size_t i = 0; i < 4; i++)
		{
			...
			squad[number][random_x][random_y] = 0; //освободи место, откуда вз€ли юнита

			//получи случайную позицию, куда юнит будет помещЄн
			random_x2 = FMath::RandRange (0, 4);
			random_y2 = FMath::RandRange (0, 4);

			//если это место зан€то другим юнитом, то перенеси его на место первого
...
float UGame_Instance_Wargame::f_distance (int index1, int index2)
{
	...
	int index = 0;
		...
		for (size_t x = 0; x < width; x++)
		{
			...
			{
				//найти рассто€ние между каждым из юнитов
	...
	return distance; //евклидово рассто€ние
}
...
void UGame_Instance_Wargame::f_Excel (float abc1, float abc2, float abc3, float abc4, float abc5)
{
	...
	if (number_battle <= 3)
	{ //подсчЄт среднего значени€ баллов в первых бо€х
	...
	if (number_stop >= 3)
	{
	File << "Optimization is over" << std::endl; //оптимизаци€ завершена
...
void UGame_Instance_Wargame::f_set_new_squad (int peshota, int maschina, int BTR, int Tank)
{
	...
	//очистка массива дл€ распределени€
	for (int x = 0; x <= 4; x++)
	{	for (int y = 0; y <= 4; y++)
		{
		squad_start[x][y] = 0;
